...
const Navbar = ({ navOpen }) => {
  ...
  const updateActiveBox = (element) => {
    ...
    const isProjectToContact =
      prevSection === "projects" && element.getAttribute("href") === "#contact";

    const isContactToProject =
      prevSection === "contact" && element.getAttribute("href") === "#projects";

    if (isLargeScreen && !(isProjectToContact || isContactToProject)) {
      // 큰 화면에서 일반적인 경우: 부드럽게 이동 (변경하지 않음)
      activeBox.current.style.transition =
        "all 0.5s cubic-bezier(0.25, 1, 0.5, 1)";
      activeBox.current.style.opacity = "1";
      activeBox.current.style.backgroundColor = isDarkMode
        ? "rgba(255, 255, 255, 1)"
        : "rgba(24, 24, 27, 1)";
  ...
  const handleClick = (event, link) => {
    ...
    const isProjectToContact =
      currentSection === "projects" && targetSection === "contact";

    const isContactToProject =
      currentSection === "contact" && targetSection === "projects";
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (initialActive) {
        ...
        activeBox.current.style.top = `${Math.round(
          rect.top - parentRect.top
        )}px`;
        activeBox.current.style.left = `${Math.round(
          ...
          if (contactLink) {
            ...
            if (isProjectToContact && window.innerWidth >= 768) {
              if (activeBox.current) {
                // 위치를 먼저 계산하고 설정
                const parentRect =
                  contactLink.parentElement.getBoundingClientRect();
          ...
          if (rect.top <= 150 && rect.bottom >= 150) {
            ...
            if (projectsLink && window.innerWidth >= 768) {
              ...
              if (activeBox.current) {
                // 위치를 먼저 계산하고 설정
                const parentRect =
                  projectsLink.parentElement.getBoundingClientRect();
        ...
        if (rect.top <= 150 && rect.bottom >= 150) {
          if (activeSection !== section) {
            ...
            if (activeLink) {
              ...
              if (
                (isProjectToContact || isContactToProject) &&
                ...
                if (activeBox.current) {
                  ...
                  activeBox.current.style.width = `${Math.round(rect.width)}px`;
                  activeBox.current.style.height = `${Math.round(
                    rect.height
                  )}px`;
    ...
    if (navOpen) {
      ...
      const updateIndicator = () => {
        ...
        if (currentActive && activeBox.current) {
          ...
          // 먼저 위치를 계산
          const parentRect =
            currentActive.parentElement.getBoundingClientRect();
