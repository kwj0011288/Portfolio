  ...
  return (
    <div className="flex flex-col md:flex-row items-start gap-4 md:gap-5 group">
      ...
      <div className="hidden md:flex md:flex-col md:items-end justify-start min-w-28 pt-2">
        ...
        <div className="dark:text-zinc-400 light:text-zinc-500 text-sm mt-1">
          {endDate}
        </div>
      </div>
      ...
      <div className="flex-1 w-full">
        ...
        <div className="flex md:hidden items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="dark:text-zinc-200 light:text-zinc-800 text-sm font-medium">
              {startDate}
            </div>
            <div className="dark:text-zinc-400 light:text-zinc-500 text-xs">
              {endDate}
            </div>
          </div>
        </div>

        <div className="rounded-2xl p-5 md:p-6 transition-all bg-white dark:bg-zinc-800/70 ring-1 ring-zinc-200 dark:ring-zinc-50/10 hover:ring-zinc-300 dark:hover:ring-zinc-50/20 hover:bg-zinc-50 dark:hover:bg-zinc-800 shadow-sm hover:shadow-md duration-300">
          {/* 데스크탑 전용 헤더 */}
          ...
          <div className="mt-0 md:mt-5 space-y-2.5 md:space-y-3.5">
            {desc.map((item, index) => (
              <p
                key={index}
                className="text-[14px] md:text-[15px] leading-relaxed flex items-start gap-2 md:gap-3 text-zinc-700 dark:text-zinc-200"
              >
          ...
          <div className="flex flex-wrap gap-1.5 md:gap-2 mt-5 md:mt-6">
            {tech.map((item, index) => (
              <span
                key={index}
                className="text-xs md:text-sm px-2 md:px-3 py-1 rounded-md bg-zinc-100 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-200"
              >
                {item}
              </span>
            ))}
